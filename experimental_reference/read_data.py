...
def load_CO_Cu_data_collection():
    ...
    for k in dat:
        if 'CO' in dat[k]:
            ...
            sel_CO, sig_CO = compute_CO_selectivity(dat[k])
            d_other.update({k:{kk:dat[k][kk] for kk in dat[k]}}) 
            U_SHE = dat[k]['V_RHE'][:,0] - (0.059*float(dat[k]['pH']))
            
            d_sel.update({k:np.array([U_SHE, sel_CO, sig_CO]).T})
...
def compute_CO_selectivity(dat):
    ...
    # error based on simple error propagation of FE-uncertainties
    sig_all = np.sqrt((np.array([jp[kk][:,1]**2.0 for kk in jp]).T).sum(axis=1))
    sig_CO = sel_CO * np.sqrt((sig_all/jp_all)**2.0 + (jp['CO'][:,1] / jp['CO'][:,0])**2.0)
    return sel_CO, sig_CO
...
def load_Ac_alloy_data(key, curr_lim, ulim=None):
    ...
    dout = {k:np.array([sdat[k]['U_SHE'][:,0], sdat[k]['SC2_%s'%Ack][:,0], sdat[k]['SC2_%s'%Ack][:,1]]).T for k in sdat}
    ...
    # roughness estimate from current at potentail (u-lim)
