...
def plot_CO2RR_CO_pot(filename, dat, ls_args, extraleg, derr={}):
    ...
    # new-fill-between
    ax.fill_between(x=dat['sim_30'][:,0], y1=dat['sim_16'][:,1], y2=dat['sim_30'][:,1], color='r', alpha=0.2)
    ...
    ax.annotate(r'$\rho$=16-30', xytext=(-1.28, 5), xy=(-1.35,10), color=clr30, size=8, # TODO: Keep?
            arrowprops=dict(arrowstyle="-", connectionstyle="arc3", color=clr30))
  # ax.annotate(r'$\rho$=16', xytext=(-1.30, 70), xy=(-1.22,55), color=clr30, size=8, 
  #         arrowprops=dict(arrowstyle="-", connectionstyle="arc3", color=clr30))
  # ax.annotate(r'$\rho$=30', xytext=(-1.28, 5), xy=(-1.35,10), color=clr30, size=8, 
  #         arrowprops=dict(arrowstyle="-", connectionstyle="arc3", color=clr30))
    
    add_sketch_tight(ax, r'CO$_2$', r'C$_{1} \!\! + \! \mathrm{C}_{2\!\!+}$', r'  CO', dxy=(0.19,0.09))
...
def make_plot_CO2RR_CO_pot(no_errorbars=True):
    ...
    clsk = {1:clrs['darkblue'], 1.1:clrs['azurblue'], 1.2:clrs['lightblue2'], 
        1.3:clrs['lightblue3'], 1.4:'b', 15:'r', 16:'r',
